...
export const build = async (client: Client, src = ".") => {
  ...
  const ctr = client
    ...
    .withMountedCache("/app/target", client.cacheVolume("wasmer-target-cache"))
...
export const deploy = async (client: Client, src = ".", cache = false) => {
  const context = client.host().directory(src);
  ...
  let baseCtr = client
    ...
    .withEnvVariable("WASMER_TOKEN", Deno.env.get("WASMER_TOKEN")!)
